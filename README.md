...
- [ ] **ScanSnap取り込みノートのタイトル改善**
  ScanSnap Home の自動採番により、取り込まれたノートのタイトルが
  `tmp__260429_0929_399688` のような形式になり、一覧で内容が判別できない。
  試行錯誤したが未解決。以下のいずれかで対処したい：
  - ScanSnap Home 側でファイル名のプレフィックスやルールを設定する
  - `scansnap_to_joplin_cli.sh` でタイトルを日付＋連番などに整形する
  - OCR結果の先頭テキストをタイトルに使う（Joplin CLI で可能か要調査）
